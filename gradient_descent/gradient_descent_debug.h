void save_progress_image(uint8_t should_save_numbered_progress_image, float *adjoined_progress_image_data, struct Node_Network *node_network, struct Node_Network_Data_Partition *node_network_data_partition, float **image_data, unsigned long long batch_size, unsigned long long image_width, unsigned long long image_height, unsigned long long image_channels, unsigned long long cycle_index)
{
    ...
    if (should_save_numbered_progress_image)
    {
