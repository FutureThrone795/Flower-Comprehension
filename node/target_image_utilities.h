...
void load_random_image(float *image_data_buffer, uint64_t image_size)
{
	int width, height, channels;
	char image_name[64];
	snprintf(image_name, 64, "cropped_102flowers/cropped_%04i.png", (rand() + rand() ^ rand()) % IMAGE_COUNT);
	...
	if (is_flipped)
	{
		for (uint64_t image_row = 0; image_row < height; image_row++)
		{
			for (uint64_t image_column = 0; image_column < width; image_column++)
			{
				for (uint64_t image_channel_index = 0; image_channel_index < channels; image_channel_index++)
				{
	...
	{
		for (uint64_t image_data_index = 0; image_data_index < image_size; image_data_index++)
		{
...
void save_image(const char* name, uint64_t width, uint64_t height, int channels, float* data)
{
...
void adjoin_images(float *image_data_buffer, float *image_data_1, float*image_data_2, uint64_t single_image_size)
{
...
void add_to_adjoined_image(float *image_data_buffer, float *image_data_1, float *image_data_2, uint64_t single_image_width, uint64_t single_image_height, uint64_t single_image_channels, uint64_t start_index)
{
	uint64_t single_image_size = single_image_width * single_image_height * single_image_channels;
	uint64_t stride_bytes = single_image_width * single_image_channels;

	for (int image_data_index = 0; image_data_index < single_image_size * 2; image_data_index++)
	{
		uint64_t single_image_data_index = image_data_index % (stride_bytes) + (image_data_index / (stride_bytes * 2)) * stride_bytes;
...
{
	uint64_t image_width = (uint64_t)ceil(sqrt(node_network->node_layers[0].input_count / 3));
	uint64_t image_height = (uint64_t)ceil(sqrt(node_network->node_layers[0].input_count / 3));

	for (uint64_t node_layer_index = 0; node_layer_index < node_network->node_layer_count; node_layer_index++)
	{
		uint64_t individual_image_side;

		individual_image_side = (uint64_t)ceil(sqrt(node_network->node_layers[node_layer_index].output_count / 3));
	...
	for (uint64_t image_data_index = 0; image_data_index < image_width * image_height * 3; image_data_index++)
	{
		image_data[image_data_index] = -2.0f;
	}

	uint64_t next_layer_image_start_index = 0;

	{
		uint64_t input_layer_image_side = (uint64_t)ceil(sqrt(node_network->node_layers[0].input_count / 3));
		for (uint64_t image_data_index = 0; image_data_index < node_network->node_layers[0].input_count; image_data_index++)
		{
	...
	for (uint64_t node_layer_index = 0; node_layer_index < node_network->node_layer_count; node_layer_index++)
	{
		uint64_t individual_image_side = (uint64_t)sqrt(node_network->node_layers[node_layer_index].output_count / 3);
		for (uint64_t image_data_index = 0; image_data_index < node_network->node_layers[node_layer_index].output_count; image_data_index++)
		{
